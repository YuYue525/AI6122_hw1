...
def sentiment_analysis(product_type="Music instruments", target_asin=None):
    ...
    total_num = sum(tag_collect.values())
    for i in tag_collect.keys():
        tag_collect[i] /= total_num
    explode = (0.1, 0, 0)
    color_set = ['#66b3ff', '#ff9999', '#ffcc99']
    plt.pie(list(tag_collect.values()), labels=(list(tag_collect.keys())),
            colors=color_set, shadow=True, autopct='%1.1f%%', explode=explode)
...
def word_cloud_for_target_asin_product(product_type="Music instruments", target_asin=None, title="00"):
    ...
    for review in review_set:
        ...
        review_re = re.sub(r'[^\w\s]', '', review_)
    ...
    print("For reviews about " + product_type + ": ")
    print('Positive words:', list(nltk.FreqDist(pos_word_list).keys()))
    print('Negative words:', list(nltk.FreqDist(neg_word_list).keys()))
